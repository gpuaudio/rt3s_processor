...
class Trainer(Module):
    ...
    def load_checkpoints(self, checkpoint_index):
        self.unwrapped_model.load(self.checkpoint_folder / f'hs-tasnet.ckpt.{checkpoint_index}.pt')

        if self.use_ema:
            self.ema_model.ema_model.load(self.checkpoint_folder /f'hs_tasnet.ema.ckpt.{checkpoint_index}.pt') # load ema
        ...
        for epoch in range(1, self.max_epochs + 1):
            ...
            if self.needs_eval:
                ...
                if self.is_main:
                    ...
                    model.save_tensor_to_file(one_eval_result_folder / 'audio.wav', eval_audio, overwrite = True)
                    model.save_spectrogram_figure(one_eval_result_folder / 'spec.png', eval_audio, overwrite = True)

                    eval_audio_paths.append(('eval_audio', str(one_eval_result_folder / 'audio.wav')))
                    ...
                    for index, (pred_target_audio, target_audio) in enumerate(zip(separated_audio, eval_targets)):
                        ...
                        saved_audio_path = one_eval_result_folder / f'target.separated.{index}.wav'
                        pred_saved_audio_path = one_eval_result_folder / f'pred.target.separated.{index}.wav'
